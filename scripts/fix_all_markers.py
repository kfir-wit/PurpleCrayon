#!/usr/bin/env python3
"""
Aggressive script to fix all marker indentation issues.
"""

import re
from pathlib import Path

def fix_markers_aggressive(file_path: Path):
    """Fix all marker indentation issues aggressively."""
    ...
    if content != original_content:
        ...
        print(f"⏭️  No marker issues in {file_path}")
        return False

def main():
    """Fix markers in all test files."""
    test_dir = Path("tests")
    test_files = list(test_dir.glob("test_*.py"))
    
    print("Fixing all marker indentation issues in test files...")
    print("=" * 60)
    
    modified_count = 0
    for test_file in test_files:
        if fix_markers_aggressive(test_file):
            modified_count += 1
    
    print("=" * 60)
    print(f"✅ Fixed markers in {modified_count} test files")
