...
def fix_indentation_comprehensive(file_path: Path):
    """Fix all indentation issues in a test file."""
    content = file_path.read_text()
    original_content = content
    
    # Fix 1: Fix markers that are incorrectly indented
    # Pattern: spaces + @pytest.mark.unit + newline + more spaces + def
    content = re.sub(
    ...
    # Fix 3: Ensure all markers are properly indented
    content = re.sub(
        r'(\s+)@pytest\.mark\.(unit|integration|stress)\n(\s+)def ',
        r'\1@pytest.mark.\2\n\1def ',
        content
    ...
    # Fix 4: Fix any remaining indentation issues
    content = re.sub(
        r'(\s+)@pytest\.mark\.(unit|integration|stress)\n(\s+)def ',
        r'\1@pytest.mark.\2\n\1def ',
        content
...
def main():
    ...
    for test_file in test_files:
        if fix_indentation_comprehensive(test_file):
            modified_count += 1
